...
import { PlusIcon, TagIcon, BanknotesIcon, UserCircleIcon, ArrowDownTrayIcon, ArrowUpTrayIcon, ArrowPathIcon, DatabaseIcon, ExclamationTriangleIcon, TrashIcon, CurrencyDollarIcon, BuildingLibraryIcon, LightBulbIcon } from './components/Icons';
  ...
  const handleImportedTransactions = (
    ...
    if (newCategories.length > 0) {
        newCategories.forEach(newCat => {
            if (!finalCategories.some(c => c.name.toLowerCase() === newCat.name.toLowerCase())) {
                const createdCat = { ...newCat, id: crypto.randomUUID() };
                addedCategories.push(createdCat);
            }
        });
        if (addedCategories.length > 0) {
          finalCategories = [...finalCategories, ...addedCategories];
          setCategories(finalCategories);
        }
    }
    
    const importedTransactions: Transaction[] = transactionsToImport.map(t_raw => {
        const { categoryName, ...trans } = t_raw;
        let finalTransaction: Transaction = { ...trans, id: crypto.randomUUID() };

        if (trans.type === 'expense' && categoryName && !trans.categoryId) {
            const foundCat = finalCategories.find(c => c.name === categoryName);
            if (foundCat) {
                finalTransaction.categoryId = foundCat.id;
            }
        }
    ...
    if (importedTransactions.length > 0) {
        setTransactions(prev => [...prev, ...importedTransactions].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
    }
  };
